...
Beyond the PWA, `trading.html` is also wrapped as a native app via
[Capacitor](https://capacitorjs.com/) — the `android/` and `ios/` directories
hold the native Android (Gradle/Android Studio) and iOS (Xcode) project
sources respectively. Both wrap the same web app: `scripts/build-www.js`
copies `trading.html` (as `index.html`), its JS modules, and the app icons
into a generated `www/` directory, which Capacitor then syncs into each
native project as the app's bundled web content.

```
npm install        # installs Capacitor deps
npm run android     # builds www/ and syncs it into android/
npm run ios         # builds www/ and syncs it into ios/
```

From there, open `android/` in Android Studio or `ios/App/App.xcodeproj`
in Xcode (on macOS) to build and run on a device or simulator. Both
`appId` and `appName` (and the app icon, generated from `resources/icon.png`
via `@capacitor/assets`) are shared from `capacitor.config.json`, so the
two platforms stay in sync automatically. Only the native project sources
are checked in — build outputs, `Pods/`, generated `public/` web assets,
and platform-specific `capacitor.config.json`/`config.xml` copies are all
gitignored and regenerated by the commands above.
